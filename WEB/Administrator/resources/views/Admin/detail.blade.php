  ...
  <div class="col-md-12">
    <div class="card">
      <div class="card-header">
        <h5 class="card-title">DATA DETAIL PRODUCT</h5>
            ...
            <div class="form-group row">
              ...
              <div class="col-sm-10">
                <div class="input-group input-group-sm">
                  <select name="ID_mapel" class="form-control select" id="eID_mapel" placeholder="-- Choose Product --" style="width: 100%;" >
                    @foreach ($mapel as $d)
              ...
              <div class="col-sm-10">
                <div class="input-group input-group-sm">
                  <select name="ID_product" class="form-control select" id="eID_product" placeholder="-- Choose Product --" style="width: 100%;" >
                    @foreach ($product as $d)
                    <option value="{{ $d->ID }}" >{{ $d->name_product }}</option>
                    @endforeach
                  </select>
                </div>
              </div>
              ...
              <div class="col-sm-10">
                <div class="input-group input-group-sm">
                  <select name="ID_user" class="form-control select" id="eID_user" placeholder="-- Choose Product --" style="width: 100%;" >
                    @foreach ($user as $d)
                    <option value="{{ $d->ID }}" >{{ $d->name_user }}</option>
                    @endforeach
                  </select>
                </div>
              </div>
              ...
              <div class="col-sm-10">
                <div class="input-group input-group-sm">
                  <input type="text" name="status" id="estatus"  class="form-control" placeholder="max 250 char" required>
                </div>
              </div>
        ...
        <form id="DelRecForm">
          ...
          <div class="modal-body">
            <div class="row">
              ...
              <dt class="col-sm-4">ID Product </dt><dd class="col-sx-1">:</dd>
              <dd class="col-sm-6" id="dID_product"></dd>
              <dt class="col-sm-4">Keterangan </dt><dd class="col-sx-1">:</dd>
              <dd class="col-sm-6" id="dketerangan"></dd>
            </div>
          </div>
...
@section('script_e')
  <script>
    ...
    $('.EditBtn').on('click',function() {
      $cID = $(this).attr("value");
      if ($cID == "new") { 
        $("#tJudul").text("Add Data Detail Product");
        $("#eID_mapel").val("");
        $("#eID_product").val("");
        $("#eID_user").val("");
        $("#estatus").val("");
        ...
        $.get("Det_Det/"+$(this).attr("value"))
          .done(function(data){
            $("#eID_mapel").val(data[0].ID_mapel);
            $("#eID_product").val(data[0].ID_product);
            $("#eID_user").val(data[0].ID_user);
