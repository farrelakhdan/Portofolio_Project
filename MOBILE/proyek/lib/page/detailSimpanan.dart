...
import 'package:proyek/data/detailSimpananItem.dart';
...
class DetailSimpanan extends StatefulWidget {
  int page;
  String norek;
  DetailSimpanan({super.key, required this.page, required this.norek});

  @override
  State<DetailSimpanan> createState() => _DetailSimpananState();
}

class _DetailSimpananState extends State<DetailSimpanan> {
  final _baseUrl = API().getBase() + API().getDetailSimpanan();

  Map data = {'jenis': "fruit"};

  late int totalpage;
  List<DetailSimpananItem> list = [];
  List color = [
    ...
    try {
      ...
      if (res['status'] == 200) {
        setState(() {
          ...
          list = it.map((e) => DetailSimpananItem.fromJson(e)).toList();
        });
        print(list.length);
      } else {
        print("error");
                  ...
                  child: Column(
                    ...
                    children: <Widget>[
                      ...
                      Row(
                        ...
                        children: <Widget>[
                          Text(
                            CurrencyFormat.convertToIdr(list[index].jumlah, 0)
                                .toString(),
                            style: TextStyle(
                                color: Colors.white,
                                fontFamily: 'avenir',
              ...
              onTap: () {
                if (widget.page == 1) {
                  ...
                  Navigator.of(context).pushReplacement(MaterialPageRoute(
                      builder: (context) => DetailSimpanan(
                            page: widget.page - 1,
                            norek: widget.norek,
                          )));
                }
