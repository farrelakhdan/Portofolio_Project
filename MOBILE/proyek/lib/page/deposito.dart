...
import 'package:proyek/data/deposito.dart';
...
class _DepositoState extends State<Deposito> {
  final _baseUrl = API().getBase() + API().getDeposito();

  List<ItemDeposito> list = [];
    ...
    try {
      ...
      if (res['status'] == 200) {
        setState(() {
          ...
          list = it.map((e) => ItemDeposito.fromJson(e)).toList();
        });
        print(list);
      } else {
        print("error");
                            ...
                            Text(
                              DateFormat('dd-MM-yyyy').format(
                                      DateFormat('yyyy-MM-dd')
                                          .parse(list[index].tgl.toString())) +
                                  " <--s/d--> " +
                                  DateFormat('dd-MM-yyyy').format(
                                      DateFormat('yyyy-MM-dd').parse(
                                          list[index].jthTmp.toString())),
                              style: TextStyle(
                                  color: Colors.white, fontFamily: 'avenir'),
                            ),
                            Text(
                              "Jenis ---------------> " +
                                  list[index].ketJenis.toString(),
                              style: TextStyle(
                                  color: Colors.white, fontFamily: 'avenir'),
                            ),
                            Text(
                              "Suku Bunga -----> " +
                                  list[index].bunga.toString() +
                                  " %",
                              style: TextStyle(
                                  color: Colors.white, fontFamily: 'avenir'),
                            ),
